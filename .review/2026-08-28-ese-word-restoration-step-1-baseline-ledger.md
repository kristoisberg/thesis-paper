...
| S05 | Study Design, sampling and annotation after the one-annotator paragraph | `paper/chapters/03_dataset_creation.tex:137-160` | Explain record fields, iterative rule revision, re-review, and washout comparison. | 250 | Use "reference annotations." State that revision was intended to improve within-annotator consistency. Preserve the repeatability-versus-correctness boundary and all current counts. |
| S06 | Study Design, project-disjoint partitioning after its opening paragraph | `paper/chapters/03_dataset_creation.tex:203-217,235-239` | Explain split roles, whole-project assignment, support filtering, and label-informed selection. | 170 | Omit equations, the full support table, the "heavily unbalanced" claim, and sample-size development history. Use current roles and values. |
| S07 | Study Design, detector configuration before the current model paragraph | `paper/chapters/04_evaluation.tex:5-21` | Explain the model-inclusion criteria used at the time. | 200 | Retain diversity and structured-output requirements. Remove state-of-the-art rankings and repeatability guarantees. Cite each retained model at first mention. |
| S08 | Study Design, expand prompt construction | `paper/chapters/04_evaluation.tex:33-58` | Explain one-request scope, training-set development, query/schema prompts, key context, preprocessing, and explicit rules. | 190 | Do not claim unpreserved improvements, infer hallucination causes, or restore full prompts. Keep synthetic Few-Shot examples clearly identified. |
| R01 | Results RQ1, after `tab:detectionErrors` | `paper/chapters/08_analysis.tex:31-35,38-40,45-49,52-58,61,71,74-81` | Add concrete code forms and examples absent from the compact table. | 220 | Use "involved" or "accounted for." Treat all reference changes as detector-informed. Remove inferred reasoning, prompt causality, and the old figure. |
