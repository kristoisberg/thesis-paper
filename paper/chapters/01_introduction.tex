...
\begin{itemize}
    ...
    \item \textbf{RQ3:} What occurrence-level agreement does one run of the developed tool achieve against the original reference annotations?
    \item \textbf{RQ4:} What is the distribution of detector flags in the identified corpus of Java projects using jOOQ?
    \item \textbf{RQ5:} Which post hoc API manifestation categories are most frequent among query-antipattern detector flags?
\end{itemize}
...
Chapter~\ref{chapter:datasetCreation} details the methodology for creating a human-annotated reference dataset, encompassing the repository mining process, project sampling, and manual annotation guidelines.
...
Chapter~\ref{chapter:projectAnalysis} outlines a framework for a large-scale project analysis, where we describe the measures used to summarise detector flags across hundreds of open-source repositories.
