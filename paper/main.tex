...
\begin{abstract}
Many SQL antipattern detectors analyse SQL statements. jOOQ instead constructs queries through Java domain-specific language expressions and generated schema classes. We evaluate a large language model (LLM)-based detector that returns class-labelled source spans and use its output to characterise flags in an identified open-source corpus. A size-stratified sample of 61 repositories yielded a single-annotator reference dataset of \num{1562} occurrences across 19 operationalised antipatterns; seven classes met the support criteria for detector analysis. Against the \num{523} original reference spans in a 20-repository, project-disjoint test partition, the selected Zero-Shot configuration with Claude Opus 4.5 achieved micro precision \num{0.858}, recall \num{0.880}, and F1-score \num{0.869}. Per-class F1 ranged from \num{0.481} to \num{0.974}. Across the 602-repository corpus, the detector produced \num{15931} flags in \num{601} repositories. ID Required flags covered \qty{49.7}{\percent} of generated-schema files and averaged \num{1.00} per flagged file; Implicit Columns flags covered \qty{24.2}{\percent} of application/query files and averaged \num{2.80} flags per flagged file. The 61 highest-count repositories contained \qty{59.1}{\percent} of flags, \qtyrange{48.1}{49.6}{\percent} of relevant files across cutoff ties, and \numrange{1.47}{1.56} times the remaining flag density. Exact-content sensitivity preserved these findings. An exploratory analysis found that the two leading Implicit Columns fragment patterns comprised \qty{71.7}{\percent} of that class's flags, while \texttt{like(} fragments comprised \qty{46.8}{\percent} of Poor Man's Search Engine flags. Agreement and output composition varied substantially by class. The corpus measures describe detector flags rather than prevalence or API-risk estimates.
...
The original study inputs and outputs, including the annotated dataset, experimental outputs, prompts, and source-material references, are publicly available at commit \href{https://github.com/kristoisberg/masters-thesis/tree/d9b35e398a6deb544f913bdbc0b211ab38474a44}{\texttt{d9b35e3}}. The normalised corpus tables are available at commit \href{https://github.com/kristoisberg/thesis-paper/tree/bb3bf6056cc7bbda29bb0fb5f0b720e7f9d06028/analysis}{\texttt{bb3bf60}} and indexed in Online Resource~1.
...
Online Resource~1 (\texttt{ESM\_1.pdf}): resource index, reproducibility record, and supporting material.
