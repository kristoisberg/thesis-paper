...
The corpus measures normalise flags by role-eligible files and report both file-pooled and repository-equal summaries. Role eligibility remains broader than a class-specific API opportunity: an application/query file may contain no eligible projection or search predicate, and a generated-schema file may expose several database objects. The measures also retain detector errors observed in the held-out evaluation. They therefore describe detector-output density, breadth, and repetition rather than antipattern prevalence or API risk. The post hoc categories likewise use returned fragments without resolved call targets or exposure denominators.
...
The preliminary sampling frame contained one duplicate repository and an incomplete path list for excluding irrelevant generated files. These conditions remain part of the sample and partitions. The corpus reconstruction corrected file accounting without changing the \num{15931} frozen detector-output rows. Model and prompt selection also used one validation partition and one observed run per configuration, making the choice sensitive to that partition and execution.
...
The corpus source snapshot survives without Git history, and its manifest preserves repository-relative paths and SHA-256 content hashes. The surviving artefacts support source-level inspection and deterministic normalisation. Exact reconstruction of the original model requests remains unavailable.
